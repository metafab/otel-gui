...
- **Keyboard navigation** — rich keyboard control: arrow keys for the span tree, `/` to search, `t`/`l`/`m` to jump to Traces/Logs/Service Map tabs, escape key to clear search and go back to the list, `?` for shortcuts help
...
| `OTEL_GUI_CORS_ORIGIN`                | `*`                | Allowed CORS origin(s) for the OTLP ingest (`/v1/*`) and read API (`/api/*`) endpoints, so browser-based OTLP exporters can post telemetry cross-origin. Use `*` to allow any origin, or a comma-separated list of exact origins (e.g. `https://app.example.com,http://localhost:5173`). For non-local/public deployments, prefer an explicit origin list to avoid exposing trace data to arbitrary websites. |
