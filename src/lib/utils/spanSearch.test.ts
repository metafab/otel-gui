...
describe('findMatchingSpanIds', () => {
	...
	it('matches on attribute value', () => {
		const span = makeSpan({ spanId: 'a', attributes: { 'http.method': 'DELETE' } });
		expect(findMatchingSpanIds([makeNode(span)], 'delete')).toContain('a');
	});

	it('matches on numeric attribute value', () => {
		const span = makeSpan({ spanId: 'a', attributes: { 'http.status_code': 500 } });
	...
	it('matches on event attribute key', () => {
		...
		expect(findMatchingSpanIds([makeNode(span)], 'exception.type')).toContain('a');
	});
	...
	it('matches on resource attribute value', () => {
		const span = makeSpan({ spanId: 'a', resource: { 'deployment.environment': 'staging' } });
	...
	it('matches on scopeName', () => {
		...
		expect(findMatchingSpanIds([makeNode(span)], 'opentelemetry')).toContain('a');
	});
