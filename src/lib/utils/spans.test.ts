...
function makeSpan(overrides: Partial<StoredSpan> & { spanId: string; parentSpanId: string; name: string }): StoredSpan {
	return {
		traceId: 'TRACE001',
...
describe('buildSpanTree', () => {
	...
	it('builds a parent-child hierarchy', () => {
		const root = makeSpan({ spanId: 'root', parentSpanId: '', name: 'root-op', startTimeUnixNano: '1000' });
		const child = makeSpan({ spanId: 'child', parentSpanId: 'root', name: 'child-op', startTimeUnixNano: '2000' });
	...
	it('places orphan spans (missing parent) as roots', () => {
		...
		const orphan = makeSpan({ spanId: 'orphan', parentSpanId: 'missing-parent', name: 'orphan-op' });
	...
	it('handles multi-level nesting', () => {
		const root = makeSpan({ spanId: 'a', parentSpanId: '', name: 'a', startTimeUnixNano: '1000' });
		const b = makeSpan({ spanId: 'b', parentSpanId: 'a', name: 'b', startTimeUnixNano: '2000' });
		const c = makeSpan({ spanId: 'c', parentSpanId: 'b', name: 'c', startTimeUnixNano: '3000' });
	...
	it('sorts children by start time', () => {
		const root = makeSpan({ spanId: 'root', parentSpanId: '', name: 'root', startTimeUnixNano: '1000' });
		const c1 = makeSpan({ spanId: 'c1', parentSpanId: 'root', name: 'c1', startTimeUnixNano: '3000' });
		const c2 = makeSpan({ spanId: 'c2', parentSpanId: 'root', name: 'c2', startTimeUnixNano: '2000' });
	...
	it('handles multiple root spans sorted by start time', () => {
		const r1 = makeSpan({ spanId: 'r1', parentSpanId: '', name: 'r1', startTimeUnixNano: '2000' });
		const r2 = makeSpan({ spanId: 'r2', parentSpanId: '', name: 'r2', startTimeUnixNano: '1000' });
	...
	it('does not infinite-loop on circular references', () => {
		// a → b → a (circular)
		const a = makeSpan({ spanId: 'a', parentSpanId: 'b', name: 'a', startTimeUnixNano: '1000' });
		const b = makeSpan({ spanId: 'b', parentSpanId: 'a', name: 'b', startTimeUnixNano: '2000' });
