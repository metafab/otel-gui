...
describe('ServiceBadge', () => {
	it('renders the service name', () => {
		const { getByText } = render(ServiceBadge, { props: { serviceName: 'frontend' } });
		expect(getByText('frontend')).toBeInTheDocument();
	});

	it('uses a <span> element', () => {
		const { container } = render(ServiceBadge, { props: { serviceName: 'api' } });
		expect(container.querySelector('span.service-badge')).toBeTruthy();
	});

	it('has a title attribute', () => {
		const { container } = render(ServiceBadge, { props: { serviceName: 'auth-service' } });
	...
	it('applies a background color via inline style', () => {
		const { container } = render(ServiceBadge, { props: { serviceName: 'payment' } });
	...
	it('renders different service names without error', () => {
		...
		for (const name of names) {
			const { getByText } = render(ServiceBadge, { props: { serviceName: name } });
