...
import { GET as getTraceList, DELETE as deleteTraces } from './api/traces/+server';
...
describe('POST /v1/traces', () => {
	...
	it('incremental ingestion: merges two batches of the same trace', async () => {
		...
		const listResponse = await getTraceList({ url: makeUrl('/api/traces') } as any);
...
describe('GET /api/traces', () => {
	...
	it('respects the limit query param', async () => {
		...
		const response = await getTraceList({ url: makeUrl('/api/traces', { limit: '3' }) } as any);
		const traces = await response.json();
	...
	it('sorts traces newest-first', async () => {
		...
		traceStore.ingest([{
			resource: { attributes: [{ key: 'service.name', value: { stringValue: 'new' } }] },
			scopeSpans: [{ scope: {}, spans: [{
...
describe('GET /api/service-map', () => {
	it('returns empty nodes and edges when no traces', async () => {
		const response = await getServiceMap({ url: makeUrl('/api/service-map') } as any);
	...
	it('scopes map to a single trace when traceId param is provided', async () => {
		...
		const listResponse = await getTraceList({ url: makeUrl('/api/traces') } as any);
		const traces = await listResponse.json();
		const singleServiceTrace = traces.find((t: any) => t.serviceName === 'frontend' && t.spanCount === 1);
